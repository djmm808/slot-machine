...
function getRandomMultiplier(): GameSymbol {
  ...
  if (rand < 0.60) return multiplierSymbols[0]; // 60% for 2x
  if (rand < 0.92) return multiplierSymbols[1]; // 32% for 5x
  return multiplierSymbols[2]; // 8% for 10x
}

// Generate random symbol (base game, no free spins)
function generateRandomSymbol(reelHasScatter: boolean): GameSymbol {
  const rand = Math.random();
...
function findClusters(grid: GameSymbol[][]): { symbol: GameSymbol; positions: [number, number][] }[] {
  ...
  for (let col = 0; col < NUM_REELS; col++) {
    for (let row = 0; row < VISIBLE_SYMBOLS; row++) {
      ...
      if (symbol.isMultiplier || symbol.isScatter) continue; // Start from regular symbols only
...
function calculateClusterWin(clusters: { symbol: GameSymbol; positions: [number, number][] }[], grid: GameSymbol[][], bet: number): number {
  let totalWin = 0;
...
function cascadeGrid(grid: GameSymbol[][], clusterPositions: [number, number][]): GameSymbol[][] {
  const newGrid: (GameSymbol | null)[][] = grid.map(col => [...col]);
  ...
  for (let col = 0; col < NUM_REELS; col++) {
    ...
    const reelHasScatter = newGrid[col].some((symbol): symbol is GameSymbol => symbol !== null && symbol.id === 200);
  ...
  return newGrid as GameSymbol[][];
}

// Simulate a single spin with cascades
function simulateSpin(bet: number): number {
...
function calculateRTP(numSpins: number = 1000000): void {
